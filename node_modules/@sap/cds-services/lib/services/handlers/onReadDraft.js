...
const { getEnrichedCQN, removeDraftUUID } = require('../utils/draftUtils')
...
const _siblingEntity = (context, columns) => {
  ...
  const targetTable = isSiblingActive ? context.target.name : `${context.target.name}_drafts`
  const columnCqnPartial = columns.map(colName => {
  ...
  if (isSiblingActive) {
    cqn
      .join(`${context.target.name}_drafts`, 'target_draft')
...
const _getUnionCQN = (context, draftName, columns, subSelect, draftWhere) => {
  ...
  if (_isOnlyCount(columns)) {
    ...
    activeDocs
      .columns(...columns)
      .where('not exists', _alignAliasForUnion(context.target.name, context.query.SELECT.from.as, subSelect))

    return union.columns({ ref: ['sum', { args: [{ ref: ['counted'] }] }], as: 'counted' })
  }
  ...
  activeDocs
    .columns(
      ...columns,
      ..._getDraftPropertiesDetermineDraft(context, hasDraftWhere, `${context.target.name}_drafts`, true)
    )
    .where('not exists', _alignAliasForUnion(context.target.name, context.query.SELECT.from.as, subSelect))
...
const _excludeActiveDraftExists = (context, draftWhere, columns) => {
  ...
  for (const key of _getTargetKeys(context)) {
    subSelect.where({ ref: [context.target.name, key] }, '=', { ref: [draftName, key] })
  }
...
const _getColumns = ({ query: { SELECT } }, model) => {
  return SELECT.columns
    ...
    : getColumns(model.definitions[SELECT.from.ref[0]]).map(col => col.name)
}
