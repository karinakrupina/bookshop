...
@after { this.attachLocation($art); }
  :
    db=TABLE? TYPE simplePath[ $name, 'Type' ]
...
hiddenWITH
  : HideAlternatives            // do not propose WITH for code completion
  | WITH
  ;
...
@after{ this.attachLocation($qp); }
  :
    id=ident['artref'] { if ($id.id) $qp.path.push($id.id); else $qp.path.broken = true; }
    ...
    |
      vp=valuePath[ 'ref', null]
      {
        ...
        if (path[0] && path[0].args) { // TODO: also arrowed args
          $expr = { op: { location: path[0].location, val: 'call' }, func: $vp.qp, args: path[0].args };
          delete path[0].args;
...
identNoKeyword[ category = 'ref' ] returns[ id ]    // for aliases without AS
